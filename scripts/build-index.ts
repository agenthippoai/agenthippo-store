...
// To make this file executable, add: #!/usr/bin/env bun
/**
 * Store Index Builder - Auto-generated by AgentHippo
 *
 * Scans agent-packs/, skills/, mcp/, engines/, and spotlight-views/ and builds an Orama search index.
...
interface StoreArtifact {
	id: string;
	type: 'pack' | 'skill' | 'mcp' | 'engine' | 'view';
	slug: string;
	displayName: string;
	description: string;
	tags: string[];
...
function resolveViewPublishVersion(manifest: Record<string, unknown>): string | undefined {
	if (typeof manifest.version === 'string' && manifest.version.trim()) {
		return manifest.version.trim().replace(/^v/, '');
	}
	if (typeof manifest.semver === 'string' && manifest.semver.trim()) {
		return manifest.semver.trim().replace(/^v/, '');
	}
	return undefined;
}

async function parseView(dir: string): Promise<StoreArtifact | null> {
	const slug = basename(dir);
	try {
		const content = await readFile(join(dir, 'view.yaml'), 'utf8');
		const manifest = parseYaml(content) as Record<string, unknown>;
		const stats = await stat(join(dir, 'view.yaml'));
		...
		return {
			...
			tags: ['spotlight', ...tags],
			author: manifest.author as string | undefined,
			latestVersion: resolveViewPublishVersion(manifest) || '0.0.0',
			updatedAt: stats.mtime.toISOString(),
		};
	} catch {
		return null;
